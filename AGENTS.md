...
- `manifest.json` declares a HACS/custom integration with `config_flow: true`, `integration_type: service`, `iot_class: cloud_polling`, and version `0.0.1-alpha`.
...
- The config flow rejects blank input, duplicate probe IDs, and non-integer probe IDs before creating an entry. It does not call RIPE Atlas.
...
- Current verified test suite covers the config flow, API client, coordinator setup, status entities/devices, bad probe setup behavior, and scaffold manifest expectations.
...
- If a configured probe returns 404 during setup, Home Assistant marks the entry as a non-retryable setup error and no entities are created. In `0.0.1-alpha`, users fix bad probe IDs by deleting and recreating the config entry.
...
- Do not add metadata sensors, changing attributes, YAML setup, API-key discovery, or reconfigure/edit flow to `0.0.1-alpha` unless the product plan changes.
